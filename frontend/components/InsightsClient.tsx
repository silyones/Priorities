                ...
                <motion.div key={selected.id}
                  ...
                  {selected.rationale.dataset && (
                    <DataCard icon={<Database className="h-3.5 w-3.5" />} label="Infrastructure dataset">
                      <div className="text-sm font-semibold text-white">{selected.rationale.dataset.name}</div>
                      <div className="text-sm text-white/50">{selected.rationale.dataset.metric}: <span className="font-semibold text-white/80">{selected.rationale.dataset.value}</span></div>
                  ...
                  {selected.rationale.demographic && (
                    <DataCard icon={<span className="text-[11px]">👥</span>} label="Demographic context (spec §8.4)">
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        <div><div className="font-mono font-bold text-white">{selected.rationale.demographic.population.toLocaleString("en-IN")}</div><div className="text-white/40">population</div></div>
                        <div><div className="font-mono font-bold text-white">{selected.rationale.demographic.literacyRate}</div><div className="text-white/40">literacy</div></div>
                        <div><div className="font-mono font-bold text-white text-[10px] leading-tight">{selected.rationale.demographic.predominantOccupation}</div><div className="text-white/40">occupation</div></div>
                      </div>
                      <div className="mt-1 text-[11px] text-white/30">Source: {selected.rationale.demographic.source}</div>
                    </DataCard>
                  )}
                  {selected.rationale.developmentPlan && (
                    <DataCard icon={<span className="text-[11px]">📋</span>} label="Development plan (spec §8.5)">
                      <div className="text-sm font-semibold text-amber-300">{selected.rationale.developmentPlan.plannedProject}</div>
                      ...
                      <div className="mt-1 text-[11px] text-white/30">Source: {selected.rationale.developmentPlan.source}</div>
                    </DataCard>
                  )}
...
function DataCard({ icon, label, children }: {
  ...
  return (
    <div className="mt-4 rounded-2xl p-4 space-y-1" style={{ background: "rgba(255,255,255,0.05)" }}>
      <span className="flex items-center gap-1.5 text-xs font-semibold text-white/60 mb-2">
        {icon} {label}
      </span>
      {children}
    </div>
