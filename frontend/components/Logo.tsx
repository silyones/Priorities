...
export function Logo({ className = "" }: { className?: string }) {
  return (
    <Link href="/" className={`group inline-flex items-center gap-2.5 ${className}`}>
      <span className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500 shadow-lift">
        <svg viewBox="0 0 24 24" className="h-5 w-5 text-night-950" fill="none">
      ...
      <span className="text-[15px] font-semibold leading-none tracking-tight text-white">
        People&apos;s
        <span className="block text-[15px] font-bold text-amber-400">Priorities</span>
      </span>
    </Link>
  );
}
